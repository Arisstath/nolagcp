...
if ($response != null && $response->success) {
    ...
    if(!empty($_POST)){ 
		...
		$num = $stmt->rowCount();
		if($num == 0){
			$data = [ 'success' => 0, 'msg' => 'Could not find that user! Please contact support.' ];
		    die(json_encode($data));
		}
		  $row = $stmt->fetch(); 
       $message = file_get_contents('../emails/passwordreset.html'); 
...
$mail->Username = 'nolagcp@mailgun.nolag.host'; // SMTP username from https://mailgun.com/cp/domains
...
$mail->Subject = 'NoLagCP password reset';
$mail->Body    = $message;
...
$query = "UPDATE users SET rtoken=:token WHERE username=:username"; 
        $query_params = array( 
            ':token' => $token,
            ':username' => $_POST['username']		
        ); 
		 $stmt = $db->prepare($query); 
            $result = $stmt->execute($query_params); 
       $data = [ 'success' => 1, 'msg' => 'Please check your emails to specify a new password!' ];
	    die(json_encode($data));
